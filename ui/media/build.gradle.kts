    ...
    sourceSets {
        ...
        desktopMain {
            dependencies {
                ...
                implementation(
                    "${libs.javafx.base.get().module}:${libs.versions.javafx.get()}:$fxClassifier"
                )
                implementation(
                    "${libs.javafx.media.get().module}:${libs.versions.javafx.get()}:$fxClassifier"
                )
                implementation(
                    "${libs.javafx.swing.get().module}:${libs.versions.javafx.get()}:$fxClassifier"
                )
                implementation(
                    "${libs.javafx.graphics.get().module}:${libs.versions.javafx.get()}:$fxClassifier"
                )
            }
        }
        iosMain {
...
macTargets.forEach { (name, target, arch) ->
    ...
    val dylibFile =
        layout.buildDirectory.file("native-libs/darwin-$arch/libAVFoundationVideoPlayer.dylib")
    tasks.register<Exec>("buildAVFoundationMac$name") {
        ...
        commandLine(
            ...
            target,
            "-o",
            dylibFile.get().asFile.absolutePath,
...
val jnaJar =
    configurations.named("desktopRuntimeClasspath").map { config ->
...
val jnaTargets = buildList {
    ...
    add(
        JnaTarget("LinuxArm", "linux-aarch64", "linux-aarch64", "libjnidispatch.so", OsFamily.LINUX)
    )
}
