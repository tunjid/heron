...
internal fun ModerationScreen(
    ...
    val threadGateSheetState =
        rememberUpdatedThreadGateSheetState(
            onDefaultThreadGateUpdated = { actions(Action.UpdateThreadGates(it)) }
        )
    ...
    LazyColumn(
        ...
        moderationToolsMenuSection(
            ...
            navigate = { actions(Action.Navigate.To(it)) },
        )
        adultLabelsSection(
    ...
    item(key = Res.string.enable_adult_content.key) {
        ElevatedItem(
            ...
            Row(
                ...
                Switch(checked = adultContentEnabled, onCheckedChange = onAdultPreferencesChecked)
            }
        }
    }
    ...
    itemsIndexed(
        ...
        itemContent = { index, labeler ->
            ...
            ElevatedItem(
                ...
                showDivider = !isLastLabel,
                onItemClicked = { onLabelerClicked(labeler) },
            ) {
                Labeler(
                    modifier = Modifier.padding(horizontal = 16.dp, vertical = 8.dp),
...
private fun LazyListScope.moderationToolsMenuSection(
    ...
    itemsIndexed(
        ...
        key = { _, tool -> tool.stringResource.key },
        itemContent = { index, tool ->
            ...
            ElevatedItem(
                ...
                ModerationItemRow(title = stringResource(tool.stringResource), icon = tool.icon)
            }
        },
    )
}
...
private fun ElevatedItem(
    ...
    if (onItemClicked == null)
        ...
        ElevatedCard(modifier = modifier, shape = shape, onClick = onItemClicked) {
            Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                content()
                if (showDivider) HorizontalDivider()
            }
...
@Composable
private fun ModerationItemRow(modifier: Modifier = Modifier, title: String, icon: ImageVector) {
    Row(
...
private enum class ModerationTools(val stringResource: StringResource, val icon: ImageVector) {
    InteractionSettings(
        stringResource = Res.string.moderation_options_interaction_settings,
        icon = Icons.Rounded.ConnectWithoutContact,
    ),
    MutedWords(stringResource = Res.string.mute_words_tags, icon = Icons.Rounded.FilterAlt),
    MutedAccounts(
