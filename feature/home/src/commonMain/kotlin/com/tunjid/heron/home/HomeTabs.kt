...
internal fun HomeTabs(
    ...
    with(sharedTransitionScope) {
        val expandableTabsState =
            rememberExpandableTabsState(tabLayout = tabLayout, onTabLayoutChanged = onLayoutChanged)
                ...
                tabs =
                    remember(timelines) {
                        timelines.map { timeline ->
                            Tab(title = timeline.name, id = timeline.sourceId, hasUpdate = false)
                        }
                    },
                selectedTabIndex = selectedTabIndex,
            ...
            if (isSignedIn)
                Row(
                    ...
                    val alphaModifier = remember {
                        Modifier.graphicsLayer { alpha = expandableTabsState.expansionProgress }
                    }
                    Text(
                        modifier = Modifier.then(alphaModifier).padding(horizontal = 8.dp),
...
private fun ExpandedTabs(
    ...
    with(sharedTransitionScope) {
            ...
            FlowRow(
                modifier =
                    with(animatedContentScope) {
                            Modifier.animateEnterExit(enter = ExpandedTabsContentEnterAnimation)
                        }
                        .fillMaxSize()
                ...
                val (pinned, saved) =
                    remember(
                        ...
                        if (index < 0) allTimelines.toList() to emptyList<Timeline.Home>()
                            ...
                            allTimelines.subList(0, index) to
                                allTimelines.subList(index, allTimelines.size)
                    }

                key(Res.string.pinned) {
                    SectionTitle(
                        modifier = Modifier.padding(top = 24.dp).animateBounds(this@with),
                    ...
                    key(timeline.sourceId) {
                        if (!editableTimelineState.isDraggedId(timeline.sourceId))
                            tabsState.ExpandedTab(
                                modifier = Modifier.animateBounds(this@with),
        ...
        LaunchedEffect(Unit) {
            snapshotFlow { currentSaveRequestState.value }
                .drop(1)
    ...
    with(sharedTransitionScope) {
        JiggleBox {
            ...
            InputChip(
                ...
                onClick = click@{
                        val index =
                            currentTimelines.indexOfFirst { it.sourceId == timeline.sourceId }
                ...
                trailingIcon = {
                    Icon(
                        modifier = Modifier.clickable { editableTimelineState.remove(timeline) },
...
@Composable
private fun DropTargetBox(modifier: Modifier = Modifier, isHovered: Boolean) {
    Box(
        modifier =
...
private fun TimelinePresentationSelector(
    ...
    val timeline = timelines.firstOrNull { it.uri == currentTabUri }
    if (timeline != null)
        Row(
            ...
            TimelinePresentationSelector(
                ...
                onPresentationSelected = { presentation ->
                    val index = timelines.indexOfFirst { it.uri == currentTabUri }
                    onTimelinePresentationUpdated(index, presentation)
                },
            )
        }
}
...
private fun Modifier.chipBackground(backgroundColor: () -> Color) = drawBehind {
...
private val ExpandedTabsShape = RoundedCornerShape(bottomStart = 16.dp, bottomEnd = 16.dp)
