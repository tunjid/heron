...
internal fun SearchScreen(
    ...
    val navigateTo = remember {
        { destination: NavigationAction.Destination -> actions(Action.Navigate.To(destination)) }
    }
    val signInPopUpState = rememberSignInPopUpState { navigateTo(signInDestination()) }

    val pagerState = rememberPagerState { state.searchStateHolders.size }
    val onProfileClicked: (Profile, String) -> Unit =
        remember(navigateTo) {
                ...
                navigateTo(
                    profileDestination(
                        ...
                        avatarSharedElementKey =
                            profile.avatarSharedElementKey(sharedElementPrefix),
    ...
    val onPostSearchResultProfileClicked =
        remember(navigateTo) {
    ...
    val sendPostInteraction = remember {
        { interaction: Post.Interaction -> actions(Action.SendPostInteraction(interaction)) }
    }

    AnimatedContent(targetState = state.layout) { targetLayout ->
        when (targetLayout) {
            ScreenLayout.Suggested ->
                SuggestedContent(
                    ...
                    trends = state.trends,
                    suggestedProfiles =
                        state.categoriesToSuggestedProfiles[state.suggestedProfileCategory]
                            ?: emptyList(),
            ...
            ScreenLayout.AutoCompleteProfiles ->
                AutoCompleteProfileSearchResults(
                    modifier = Modifier.fillMaxSize().padding(top = 16.dp),
    ...
    LifecycleStartEffect(Unit) {
        actions(Action.FetchSuggestedProfiles(category = state.suggestedProfileCategory))
        onStopOrDispose {}
    }
}
