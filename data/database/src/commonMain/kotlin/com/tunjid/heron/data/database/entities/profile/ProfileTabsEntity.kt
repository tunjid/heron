...
fun ProfileTabsEntity?.asExternalModels(): List<ProfileTab> =
    when (this) {
        null -> Default
            ...
            if (delimitedTabs.isBlank()) Default
            else
                delimitedTabs.split(TabDelimiter).mapNotNull(String::asProfileTabOrNull).ifEmpty {
                    Default
                }
    }

private fun String.asProfileTabOrNull(): ProfileTab? =
    when (this) {
        ...
        StandardDocumentUri.NAMESPACE -> ProfileTab.StandardSite.Documents
            ...
            when (val uri = asRecordUriOrNull()) {
                is FeedGeneratorUri -> ProfileTab.Bluesky.FeedGenerators.FeedGenerator(uri)
                else -> null
            }
    }

fun List<ProfileTab>.profileTabsEntity(profileId: ProfileId) =
    ProfileTabsEntity(
        profileId = profileId,
        delimitedTabs =
            joinToString(
...
operator fun ProfileTabsEntity.plus(tab: String) =
    when (tab.asProfileTabOrNull()) {
        null -> this
        else ->
            copy(
                delimitedTabs =
                    if (delimitedTabs.isBlank()) tab else "$delimitedTabs$TabDelimiter$tab"
            )
    }

private val Default: List<ProfileTab> =
    listOf(
