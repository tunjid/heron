...
internal object Migration42To43RockskyCreatorId : Migration(42, 43) {
    ...
    override fun migrate(connection: SQLiteConnection) {
        ...
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_rockskyAlbums_creatorId` ON `rockskyAlbums` (`creatorId`)"
        )
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_rockskyArtists_creatorId` ON `rockskyArtists` (`creatorId`)"
        )
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_rockskyTracks_creatorId` ON `rockskyTracks` (`creatorId`)"
        )
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_rockskyScrobbles_creatorId` ON `rockskyScrobbles` (`creatorId`)"
        )
    }
}
