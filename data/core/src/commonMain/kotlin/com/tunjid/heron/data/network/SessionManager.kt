...
internal interface SessionManager {
    ...
    suspend fun initiateOauthSession(request: OauthUriRequest): SavedState.AuthTokens.Pending

    suspend fun createSession(request: SessionRequest): SavedState.AuthTokens
...
@Inject
constructor(httpClient: HttpClient, private val savedStateDataSource: SavedStateDataSource) :
    SessionManager {

    private val sessionRequestUrl = MutableStateFlow<Url?>(null)

    private val authHttpClient =
        httpClient.config {
    ...
    private val api = XrpcBlueskyApi(httpClient = authHttpClient)

    private val oAuthApi: OAuthApi = OAuthApi(client = authHttpClient)
    ...
    override suspend fun createSession(request: SessionRequest): SavedState.AuthTokens =
        try {
            ...
            when (request) {
                    ...
                    api.createSession(
                        ...
                        .map { result ->
                            SavedState.AuthTokens.Authenticated.Bearer(
                                ...
                                didDoc =
                                    SavedState.AuthTokens.DidDoc.fromJsonContentOrEmpty(
                ...
                is SessionRequest.Oauth -> {
                    val existingAuth =
                        withTimeoutOrNull(PendingTokenTimeout) {
                    ...
                    val code =
                        callbackUrl.parameters[OauthCallbackUriCodeParam]
                ...
                is SessionRequest.Guest -> SavedState.AuthTokens.Guest(server = request.server)
            }
        } finally {
    ...
    override fun manage(config: HttpClientConfig<*>) =
        with(config) {
            install(DefaultRequest) {
                    ...
                    saveAuth = { tokens ->
                        when (val sessionContext = currentSessionContext()) {
                            is SessionContext.Previous -> {
                                ...
                                if (tokens != null)
                                    savedStateDataSource.updateAuth(
                                        profileId = profileId,
                                        auth = tokens,
                                    )
    ...
    private suspend fun refresh(
        ...
        when (tokens) {
            is SavedState.AuthTokens.Authenticated.Bearer ->
                authHttpClient
                    .post(urlString = RefreshTokenEndpoint, block = { bearerAuth(tokens.refresh) })
...
private fun atProtoAuth(
    ...
    createClientPlugin("AtProtoAuthPlugin") {
        ...
        val maybeUpdateAndSaveDPoPNonce:
            suspend SavedState.AuthTokens.Authenticated?.(String?) -> Unit =
            update@{ newNonce ->
                this ?: return@update
                newNonce ?: return@update
                nonceDeferredMutex
                    .withSingleAccess(newNonce) { maybeUpdateDPoPNonce(newNonce) }
                    ?.also { saveAuth(it) }
            }

        val proceedAndRetryIfDPoPNonceError:
            suspend Send.Sender.(
                existingTokens: SavedState.AuthTokens.Authenticated?, context: HttpRequestBuilder,
            ) -> HttpClientCall =
            call@{ tokens, context ->
                ...
                if (error == UseDPoPNonce) {
                    val updatedTokens =
                        call.newDPoPNonce?.let(tokens::maybeUpdateDPoPNonce) ?: return@call call
        ...
        on(Send) intercept@{ context ->
            ...
            if (ChatProxyPaths.any(predicate = context.url.encodedPath::endsWith)) {
                ...
                authTokens?.serviceUrl?.let { context.url.set(host = Url(urlString = it).host) }
            }

            if (
                authTokens == null &&
                    SignedOutPaths.any(predicate = context.url.encodedPath::endsWith)
            ) {
                context.url.set(host = Url(urlString = SignedOutUrl).host)
            } else if (
                authTokens != null &&
                    !ChatProxyPaths.any(predicate = context.url.encodedPath::endsWith)
            ) {
            ...
            var result: HttpClientCall = proceedAndRetryIfDPoPNonceError(authTokens, context)
                    ...
                    ExpiredTokenError ->
                        ...
                        when (result.response.status) {
                            HttpStatusCode.Unauthorized ->
                                if (authTokens != null) throw InvalidTokenException() else null
                            else -> null
                        }
                }
            }
            ...
            updatedTokensResult.fold(
                onSuccess = { updatedTokens ->
                    if (updatedTokens != null) {
                        ...
                        context.authenticate(updatedTokens)
                        result =
                            proceedAndRetryIfDPoPNonceError(updatedTokens, context).also { call ->
                                when {
                ...
                onFailure = {
                    ...
                    when (it) {
                        ...
                        is AtpException,
                        is AtProtoException -> saveAuth(null)
                        else -> throw it
                    }
                },
            )
...
private suspend fun HttpClientCall.atProtoError() =
    runCatching<AtpErrorDescription?> { response.body<AtpErrorDescription>() }.getOrNull()?.error

private fun SavedState.AuthTokens.Authenticated?.maybeUpdateDPoPNonce(
    newNonce: String
): SavedState.AuthTokens.Authenticated.DPoP? {
    return if (this is SavedState.AuthTokens.Authenticated.DPoP) copy(nonce = newNonce) else null
}
...
private suspend fun OAuthToken.toAppToken(authEndpoint: String) =
    SavedState.AuthTokens.Authenticated.DPoP(
        ...
        pdsUrl = pds.toString(),
        keyPair =
            SavedState.AuthTokens.Authenticated.DPoP.DERKeyPair(
...
private val HeronOauthScopes =
    listOf(OAuthScope.AtProto, OAuthScope.Generic, OAuthScope.BlueskyChat)

private val ChatProxyPaths =
    listOf(
