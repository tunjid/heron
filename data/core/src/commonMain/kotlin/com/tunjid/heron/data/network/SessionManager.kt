        ...
        on(Send) intercept@{ context ->
            ...
            if (context.url.encodedPath.contains(ComAtProtoPathSegment)) {
                context.extractSubjectDid()?.let { subjectDid ->
                    val isSignedInUser =
                        authTokens != null && authTokens.authProfileId.id == subjectDid.did
                    if (!isSignedInUser) {
            ...
            val shouldAuthenticate =
                !resolvedOtherProfilePds && !context.headers.contains(Authorization)

            if (shouldAuthenticate && authTokens != null) {
                context.authenticate(authTokens)
            }

            var result: HttpClientCall =
                proceedAndRetryIfDPoPNonceError(
                    authTokens,
                    context,
                )
...
private fun HttpRequestBuilder.extractSubjectDid(): Did? =
    extractDidFromQueryParams()?.let(::Did) ?: extractDidFromBody()?.let(::Did)

private fun HttpRequestBuilder.extractDidFromQueryParams(): String? =
    url.parameters[DidParam] ?: url.parameters[RepoParam]?.takeIf { it.startsWith(DidPrefix) }
...
private fun HttpRequestBuilder.extractDidFromBody(): String? {
    ...
    return (jsonObject[DidParam] as? JsonPrimitive)?.content
        ?: (jsonObject[RepoParam] as? JsonPrimitive)?.content?.takeIf { it.startsWith(DidPrefix) }
}
