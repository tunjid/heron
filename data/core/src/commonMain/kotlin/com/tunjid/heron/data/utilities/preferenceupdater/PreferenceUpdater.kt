...
internal class ThingPreferenceUpdater @Inject constructor(private val tidGenerator: TidGenerator) :
    PreferenceUpdater {
        ...
        networkPreferences.fold(
            ...
            operation = { foldedPreferences, preferencesUnion ->
                when (preferencesUnion) {
                    is PreferencesUnion.AdultContentPref ->
                        foldedPreferences.copy(allowAdultContent = preferencesUnion.value.enabled)
                    is PreferencesUnion.BskyAppStatePref -> foldedPreferences
                    is PreferencesUnion.ContentLabelPref ->
                        foldedPreferences.copy(
                            contentLabelPreferences =
                                foldedPreferences.contentLabelPreferences.plus(
                    ...
                    is PreferencesUnion.FeedViewPref ->
                        foldedPreferences.copy(
                            feedPreferences =
                                foldedPreferences.feedPreferences +
                                    preferencesUnion.asExternalModel()
                        )
                    is PreferencesUnion.HiddenPostsPref ->
                        foldedPreferences.copy(
                            hiddenPostPreferences =
                                preferencesUnion.value.items.map {
                                    HiddenPostPreference(uri = it.atUri.let(::PostUri))
                                }
                        )
                    is PreferencesUnion.InterestsPref -> foldedPreferences
                    is PreferencesUnion.LabelersPref ->
                        foldedPreferences.copy(
                            labelerPreferences =
                                preferencesUnion.value.labelers.map {
                                    LabelerPreference(
                                        labelerCreatorId = it.did.did.let(::ProfileId)
                                    )
                                }
                        )
                    is PreferencesUnion.MutedWordsPref ->
                        foldedPreferences.copy(
                            mutedWordPreferences =
                                preferencesUnion.value.items.map {
                                    MutedWordPreference(
                                        ...
                                        actorTarget =
                                            it.actorTarget?.value?.let(MutedWordPreference::Target),
                    ...
                    is PreferencesUnion.ThreadViewPref ->
                        foldedPreferences.copy(
                            threadViewPreferences =
                                ThreadViewPreference(sort = preferencesUnion.value.sort?.value)
                        )
                    is PreferencesUnion.Unknown -> foldedPreferences
                    is PreferencesUnion.PostInteractionSettingsPref ->
                        foldedPreferences.copy(
                            postInteractionSettings =
                                PostInteractionSettingsPreference(
                                    threadGateAllowed =
                                        when (
                                            val allow = preferencesUnion.value.threadgateAllowRules
                                        ) {
                                            // All can reply
                                            null -> null
                                            // None can reply
                                            emptyList<BskyPostReplyGateRule>() ->
                                                ThreadGate.Allowed(
                                                ...
                                                allow
                                                    .groupBy { it::class }
                                                    .let { grouped ->
                                                        // ThreadgateAllowUnion.ListRule is handled
                                                        // with the list views above
                                                        ThreadGate.Allowed(
                                                            allowsFollowing =
                                                                BskyPostReplyGateRule
                                                                    .FollowingRule::class in
                                                                    grouped,
                                                            allowsFollowers =
                                                                BskyPostReplyGateRule
                                                                    .FollowerRule::class in grouped,
                                                            allowsMentioned =
                                                                BskyPostReplyGateRule
                                                                    .MentionRule::class in grouped,
                                                            allowedListUris =
                                                                grouped[
                                                                        BskyPostReplyGateRule
                                                                            .ListRule::class]
                                    ...
                                    allowedEmbeds =
                                        preferencesUnion.value.postgateEmbeddingRules?.let {
                                            it.fold(
                                                ...
                                                operation = { allowed, value ->
                                                    when (value) {
                                                        ...
                                                        is BskyPostEmbedGateRule.Unknown -> allowed
                                                    }
                                                },
                                            )
                                        },
                    ...
                    is PreferencesUnion.VerificationPrefs ->
                        foldedPreferences.copy(
                    ...
                    is PreferencesUnion.DeclaredAgePref ->
                        foldedPreferences.copy(
                            declaredAgePreferences =
                                DeclaredAgePreference(
                                    minAge =
                                        when {
    ...
    override suspend fun update(
        ...
        val groupedPreferences = networkPreferences.groupBy(keySelector = { it::class })
        val targetClass = update.targetClass()
        val targetPreferences =
            groupedPreferences.getOrElse(
                key = targetClass,
                defaultValue = ::emptyList,
            )
        val updatedPreferences = update.updatePreferences(existingPreferences = targetPreferences)
                ...
                SavedFeedsPrefV2(
                    items =
                        when (this) {
                            ...
                            is Timeline.Update.HomeFeed.Pin ->
                                preferenceUnion.value.items
                                    .filter {
                                        it.value != uri.uri
                                    }
                            ...
                            is Timeline.Update.HomeFeed.Remove ->
                                preferenceUnion.value.items.filter { savedFeed ->
                                    savedFeed.value != uri.uri
                                }

                            is Timeline.Update.HomeFeed.Save ->
                                preferenceUnion.value.items.filter {
                                    it.value != uri.uri
                                } +
                                    SavedFeed(
                                        id = tidGenerator.generate(),
    ...
    ): List<PreferencesUnion.ContentLabelPref> =
        when (this) {
                is Timeline.Update.OfContentLabel.AdultLabelVisibilityChange ->
                    label.labelValues.map { label ->
                        PreferencesUnion.ContentLabelPref(
                            value =
                                ContentLabelPref(
                                    ...
                                    label = label.value,
                                    visibility =
                                        ContentLabelPrefVisibility.safeValueOf(visibility.value),
                                )
                        )
                    }
                is Timeline.Update.OfContentLabel.LabelVisibilityChange ->
                    listOf(
                        PreferencesUnion.ContentLabelPref(
                            value =
                                ContentLabelPref(
    ...
    /**
     * A labeler preference may or may not already exist for a particular update. If subscribed,
     * this method unconditionally creates the preference, and filters out an existing one if
     * present. If not subscribed, the existing one is simply removed.
     */
    private fun Timeline.Update.OfLabeler.updateLabelerPreferences(
        ...
        val labelers = existingPreferences.flatMap { it.value.labelers }
        // Return a singleton list
        return listOf(
            PreferencesUnion.LabelersPref(
                value =
                    LabelersPref(
                        labelers =
                            when (this) {
                                is Timeline.Update.OfLabeler.Subscription ->
                                    when {
                                        subscribed ->
                                            labelers
                                                .plus(
                                                    LabelerPrefItem(
                                                        did = labelCreatorId.id.let(::Did)
                                                    )
                                                )
                                                .distinctBy(LabelerPrefItem::did)
                                        else -> labelers.filter { it.did.did != labelCreatorId.id }
                                    }
                            }
                    )
            )
    ...
    private fun Timeline.Update.OfAdultContent.updateAdultContentPreferences():
        List<PreferencesUnion.AdultContentPref> =
        // Return a singleton list

        listOf(PreferencesUnion.AdultContentPref(AdultContentPref(enabled = enabled)))

    private fun Timeline.Update.OfMutedWord.updateMutedWordPreferences():
        List<PreferencesUnion.MutedWordsPref> =
        when (this) {
            is Timeline.Update.OfMutedWord.ReplaceAll ->
                listOf(
                    PreferencesUnion.MutedWordsPref(
                        value =
                            MutedWordsPref(
                                items =
                                    mutedWordPreferences.map { pref ->
                                        MutedWord(
                                            id = null,
                                            value = pref.value,
                                            targets =
    ...
    private fun Timeline.Update.OfInteractionSettings.updatePostInteractionPreferences(
        ...
        val threadGateAllowRules =
            preference.threadGateAllowed?.let { allowed ->
                buildList {
                    if (allowed.allowsFollowing)
                        add(BskyPostReplyGateRule.FollowingRule(ThreadgateFollowingRule))
                    if (allowed.allowsFollowers)
                        add(BskyPostReplyGateRule.FollowerRule(ThreadgateFollowerRule))
                    if (allowed.allowsMentioned)
                        add(BskyPostReplyGateRule.MentionRule(ThreadgateMentionRule))
        ...
        val postgateEmbeddingRules =
            if (preference.allowedEmbeds.embedsDisabled) {
    ...
    ): List<PreferencesUnion.FeedViewPref> =
        when (this) {
    ...
    private fun Timeline.Update.OfThreadViewPreference.updateThreadViewPreferences():
        List<PreferencesUnion.ThreadViewPref> =
        when (this) {
                ...
                listOf(
                    PreferencesUnion.ThreadViewPref(
                        value =
                            ThreadViewPref(
                                sort =
                                    when (threadViewPreference.order()) {
                                        TimelineItem.Threaded.Order.Oldest ->
                                            ThreadViewPrefSort.Oldest
                                        TimelineItem.Threaded.Order.Newest ->
                                            ThreadViewPrefSort.Newest
                                        TimelineItem.Threaded.Order.Top ->
                                            ThreadViewPrefSort.MostLikes
                                    }
                            )
                    )
                )
    ...
    private suspend fun Timeline.Update.updatePreferences(
        ...
        when (this) {
            is Timeline.Update.HomeFeed ->
                updateHomeTimelinePreferences(
                    existingPreferences =
                        existingPreferences.filterIsInstance<PreferencesUnion.SavedFeedsPrefV2>()
                )
            is Timeline.Update.OfAdultContent -> updateAdultContentPreferences()
            is Timeline.Update.OfContentLabel ->
                updateContentLabelPreferences(
                    existingPreferences =
                        existingPreferences.filterIsInstance<PreferencesUnion.ContentLabelPref>()
                )
            is Timeline.Update.OfLabeler ->
                updateLabelerPreferences(
                    existingPreferences =
                        existingPreferences.filterIsInstance<PreferencesUnion.LabelersPref>()
                )
            is Timeline.Update.OfMutedWord -> updateMutedWordPreferences()
            // Only one of these should exist
            is Timeline.Update.OfInteractionSettings ->
                updatePostInteractionPreferences(
                    existingPreference =
                        existingPreferences
                            .filterIsInstance<PreferencesUnion.PostInteractionSettingsPref>()
                            .firstOrNull()
                )
            is Timeline.Update.OfFeedPreference ->
                updateFeedPreferences(
                    existingPreferences =
                        existingPreferences.filterIsInstance<PreferencesUnion.FeedViewPref>()
                )
...
private fun Timeline.Update.targetClass() =
    when (this) {
        ...
        is Timeline.Update.OfInteractionSettings ->
            PreferencesUnion.PostInteractionSettingsPref::class
        is Timeline.Update.OfFeedPreference -> PreferencesUnion.FeedViewPref::class
        is Timeline.Update.OfThreadViewPreference.ThreadView ->
            PreferencesUnion.ThreadViewPref::class
    }

private fun PreferencesUnion.ContentLabelPref.asExternalModel() =
