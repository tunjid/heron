...
internal class AuthTokenRepository(
    ...
    override val isSignedIn: Flow<Boolean> =
        savedStateDataSource.signedInAuth.map { it != null }.distinctUntilChanged()

    override val isGuest: Flow<Boolean> =
        savedStateDataSource.savedState
            .map { savedState -> savedState.auth is SavedState.AuthTokens.Guest }
    ...
    override suspend fun createSession(request: SessionRequest): Outcome =
        runCatchingUnlessCancelled { sessionManager.createSession(request) }
                ...
                savedStateDataSource.inCurrentProfileSession { currentProfileId ->
                    ...
                    val freshAuth =
                        savedStateDataSource.inPastSession(sessionSummary.profileId) {
                            networkService
                                .runCatchingWithMonitoredNetworkRetry { getSession() }
                                .getOrNull()
    ...
    override suspend fun signOut() {
        runCatchingUnlessCancelled { sessionManager.endSession() }
    ...
    override suspend fun updateSignedInUser(): Outcome =
        savedStateDataSource.inCurrentProfileSession { signedInProfileId ->
            ...
            networkService
                .runCatchingWithMonitoredNetworkRetry { getSession() }
                .fold(onSuccess = { updateSignedInUser(it.did) }, onFailure = Outcome::Failure)
        } ?: expiredSessionOutcome()
    ...
    private suspend fun savePreferences(preferencesResponse: GetPreferencesResponse) =
        supervisorScope {
            ...
            val saveTimelinePreferences = async {
                savedStateDataSource.updateSignedInProfileData { copy(preferences = preferences) }
            }
            val types =
            ...
            val lists =
                types[SavedFeedType.List.value]?.map {
                    async {
                        networkService.runCatchingWithMonitoredNetworkRetry(times = 2) {
                            getList(
                                GetListQueryParams(cursor = null, limit = 1, list = AtUri(it.value))
                            )
                        }
                    }
                } ?: emptyList()
    ...
    private suspend fun saveNotificationPreferences(
        ...
        savedStateDataSource.updateSignedInUserNotifications {
            copy(preferences = updatedNotifications.preferences, lastRefreshed = Clock.System.now())
        }
    }
}
